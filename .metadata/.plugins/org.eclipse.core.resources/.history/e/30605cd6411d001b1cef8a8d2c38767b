...
global {
    ...
    point storePoint <-  {rnd (0.0, 100.0), rnd (0.0, 100.0)};
    
    int info_range <- 2;
    
    init {
    	create FestivalGuest number: nb_guests_init ;
    	create FestivalStore number: 1 ;
...
species FestivalGuest skills:[moving]{
	...
	reflex moveToInformation when: (targetPoint = nil and (hunger >= 10 or thirst >= 10 or tired >= 10)){
    ...
    reflex die when: (hunger >= 10 and thirst >= 10 and tired >= 10) {
    	do debug(name + " will disappear from the simulation");
    	do die ;
    }
} 
...
species FestivalStore {
    ...
    point location <- storePoint;
    
    image_file my_icon <- image_file("../includes/images/store.png");
    aspect icon {
        draw my_icon size: 5*size;
    }
...
species FestivalInformationCenter {
    ...
    image_file my_icon <- image_file("../includes/images/information.png");
    aspect icon {
        draw my_icon size: 5*size;
    }
    ...
    point storeLocation <- storePoint;
    
    reflex inform when: !empty(FestivalGuest at_distance info_range){
    	ask FestivalGuest at_distance info_range{
    		if(self.targetPoint = myself.location){
    			self.targetPoint <- myself.storeLocation;
    		}
    	}
    }
...
experiment FestivalMain type: gui {
	...
	output {
    	display main_display {
        	...
        	species FestivalStore aspect: icon ;
        	species FestivalInformationCenter aspect: icon ;
    	}
